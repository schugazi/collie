...
- **Menu Cancel works on the first tap.** Cancel sends Esc, which commits nothing, so it now takes the same same-menu check as the arrows; before, a Cancel tapped just after an arrow was refused as a changed screen and needed a second tap.
...
- **The menu arrows keep their size when the effort changes.** The `/model` picker's arrow row drops the effort label that sat between ← and →, which the mirror above already shows, and its four arrows each take a quarter of the row.
...
- **The home screen's workspace strip keeps its height.** The strip keeps the padding its chips' tap area reaches into, and gives that padding back around the row, so the dashboard's list starts where it did.
...
- **A Claude question shows its buttons while the task list is open.** While a dialog is up, Claude Code prints its task list under the dialog's footer, so Collie took the footer for scrolled-away output and drew the "cannot read this dialog" card instead of the question. Every Claude dialog grammar now sets the task list aside before it looks for the footer, the bridge's tap check measures its window from above the list, only a list whose count matches its rows is set aside, and a wrapped question no longer shows the `│` bar Claude draws beside it. Codex questions were checked and are not affected.
- **A diff line Claude wrapped itself keeps the diff's indent on the phone.** Claude continues a long diff line on rows that carry the `+` or `-` but no line number, and those rows lined up one column off: under the sign, or past it when the text opened with a space. They now line up under the code, like the numbered row above them.
- **A wrapped diff line on the phone is one solid band of colour.** The green or red fill used to sit only behind the characters, so it started at the line number on the first line and at the code on the rest, ended raggedly on the right, and showed dark seams between rows. Each row now fills edge to edge from where Claude's fill starts, like the terminal, and an empty added or removed line gets its band too.
- **Replies reach a Codex pane in fullscreen mode again.** With `fullscreen_transcript` on, Codex prints a `? for shortcuts` row under its status line while the input box is empty, so Collie could not find the box on an idle or working pane: every send was refused and the "cannot read this dialog" card covered the mirror. Collie now steps over that one row, strips it with the input box, and leaves it out of the status strip.
...
- **A Claude pane running a background shell takes replies again.** Claude's mode row now ends in `1 shell · ↓ to manage`, and Collie read that hint as a dialog's footer, so it lost the input box, refused every send and covered the mirror with the "cannot read this dialog" card. That one hint is now read as part of the status line.
- **Claude and Codex panes stop showing the dialog card over a live input box.** With no statusline configured, Claude's footer also prints `esc to interrupt` while working and `ctrl+t to hide tasks` with open tasks, cut short with `…` on a narrow pane, and Codex 0.157 prints `← for agents · ? for shortcuts` or `tab to queue message` under every composer. Each read as a dialog's keys, so sends were refused and the "cannot read this dialog" card covered the mirror. Those footer hints now count as composer chrome.
