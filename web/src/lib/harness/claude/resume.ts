...
import { dialogTail, isBlank, lineText } from "./markers";
...
export function detectResumePickerRegion(lines: StyledLine[]): PromptRegion | null {
  ...
  // 1. The footer, read across the rows the terminal wrapped it onto, naming the way out — above any
  //    task panel Claude parks under the dialog (markers.ts `dialogTail`).
