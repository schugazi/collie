...
describe("the real captures lift as a list of sessions", () => {
  ...
  // Claude parks its task panel under an open dialog (markers.ts `dialogTail`). No /resume capture
  // shows one, so this composes the real w60 picker with the panel shape the other captures print.
  it("w60-first: a task panel under the footer is set aside", () => {
    const panel = ["", "  3 tasks (1 done, 2 open)", "  ✔ Phase A", "  ◻ Phase B", "  ◻ Phase C"];
    const model = detectResumePicker(
      fromTexts([...textsOf("claude--menu-resume-picker--w60-first.txt"), ...panel]),
    )!;
    expect(model.options.map((o) => o.label)).toEqual([...LAB_SESSIONS, "Cancel"]);
    expect(model.options[3]!.keys).toEqual(["Down", "Down", "Down", "Enter"]);
    expect(model.signature.endsWith("· Type to search · Esc to cancel")).toBe(true);
